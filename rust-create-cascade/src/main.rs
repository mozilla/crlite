...
fn count_all(
    ...
    if let Some(output_csv_path) = output_csv_path {
        ...
        writeln!(output_csv, "issuer spki hash, unspecified, key_compromise, privilege_withdrawn, affiliation_changed, superseded, cessation_of_operation, revoked certificates, non-revoked certificates").expect("could not write reason code count line");
