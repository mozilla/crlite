...
func (c *CTConfig) Usage() {
	...
	fmt.Printf("")
	fmt.Printf("Config file directives:")
	fmt.Printf("")
	fmt.Printf("certPath = Path under which to store full DER-encoded certificates")
	fmt.Printf("issuerCNFilter = Prefixes to match for CNs for permitted issuers, comma delimited")
	fmt.Printf("runForever = Run forever, pausing `pollingDelay` between runs")
	fmt.Printf("pollingDelay = Wait this many minutes between polls")
	fmt.Printf("logExpiredEntries = Add expired entries to the database")
	fmt.Printf("numThreads = Use this many threads per CPU")
	fmt.Printf("logList = URLs of the CT Logs, comma delimited")
}
